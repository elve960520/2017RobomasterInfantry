...
u8 CAN1_Send_Msg_yaw(u8* msg,u8 len)						//发送数据
{	
  u8 mbox;
  u16 i=0;
  CanTxMsg TxMessage;
...
//返回值:0,无数据被收到;
//		 其他,接收的数据长度;
u8 CAN1_Receive_Msg(u8 buf[5][8])
{		   		   	   		   
 	u32 i;
	u8 n;
	CanRxMsg RxMessage;
  if(CAN_MessagePending(CAN1,CAN_FIFO0)==0) return 0;		//没有接收到数据,直接退出 
	...
	if(RxMessage.StdId == 0x201) n = 0;
	else if(RxMessage.StdId == 0x202) n = 1;
	else if(RxMessage.StdId == 0x203) n = 2;
	else if(RxMessage.StdId == 0x204) n = 3;
	else n = 4;
  for(i=0;i<RxMessage.DLC;i++)
    buf[n][i]=RxMessage.Data[i]; 
